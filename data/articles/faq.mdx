...
The sources of this article can be found at the end of this page. Thanks to all the high value contenthas resulted in this FAQ!
...
Recently, new image formats with more efficient compression capabilities than JPEG have been developed. However, these formats have some shortcomings concerning the use cases discussed below. Several evidence-based compression algorithms have recently been presentedoutperform the currently used standards for image coding.

While these technological advances have been made, no widely accepted standard existscan support state-of-the-art compression performance and is widely accepted by consumer devices and browsers alike.
...
In addition to having a file extension of JXL, JPEG XL has the internet media type image/jxl. JXL files are images saved in the JPEG XL file format developed by JPEG (Joint Photographic Experts Group). Files contain a photo, illustration, screenshot, or other raster image datahas been compressed using the JPEG XL compression algorithm.
...
XL shares the same disadvantage as any new formatneeds to be established: It lacks support. In addition, few operating systems support JPEG XL. There is a lack of browsers and image viewerssupport it even though many browsers added support for JPEG XL behind a flag.
...
Please notethis list may or may not be complete.
...
A Call for Proposals for JPEG XL, a next-generation image compression standard, was issued in 2017 by JTC1/SC29/WG1. On December 25, 2020, the bitstream format (file format) was frozen, ensuringit will be decodable by future releases.
...
Both Google and Cloudinary developed a combination of the PIK and FUIF formats to create JPEG XL. But JPEG XL can'tbe considered a combination of these two formats. After several years of joint development, PIK and FUIF were combined, and many improvements were made.
...
- ISO/IEC 18181-2. Part 2, File format: Specifies an extensible box-based file formatsupports metadata (e.g., EXIF and JUMBF) and legacy JPEG bitstream reconstruction data. (released)
...
In the past, several attempts at dethroning the old JPEG have failed. JPEG XL's creators are hopefulthey have succeeded this time and have created a worthy successor to a 30-year-old image formatis as old as the internet.
...
Progression of saliency: In progressive scans of JPEGs, each image part is given the same amount of detail. This is not the case for JPEG XL.means you can encode images progressively based on saliency, such as encoding faces or foreground objects in more detail first, followed by the background.
...
- JPEG XL is intended to be a formatoffers high fidelity and a small file size. Therefore, it can support many high-quality applications, including wide gamut / HDR, visually lossy 4:4:4 up to 32 bits of depth, dynamic range, and generation loss resilience.
- Lossless up to 32 bits per channel (float or int)
- Compatible with HDR & wide gamut
- Notfor end-user image delivery but also authoring workflows
...
In a presentation about JPEG XL, I've found a funny exampledisplays how efficient JPEG XL is:
...
The JPEG format is a single-core format. On multicore platforms, JPEG XL is faster to encode and decode. Hardware is not necessary; software encodes and decodes are fast enough. libjpeg turbo has a decoding speed of 200 MP/s, and mozjpeg has 100 MP/s. JPEG XL has a decoding speed of 30 MP/s, but with eight cores,results in 240 MP/s.

## Progressive image based on DCT

A JPEG XL bitstream may contain multiple passes of AC coefficientsare summed together. Gradually encoding images is possible by sending low-frequency coefficients first and higher frequency coefficients later. A particular section of the image can also be refined in a given pass, allowing for area-of-interest progressive decoding.
...
The JPEG XL format is a superset of the JPEG format. In addition, JPEG XL developers did not change any of the JPEG coding tools, even with its numerous under-the-hood coding toolsachieve superior compression. Therefore, all JPEG image data is represented as JPEG XL.
...
As a result, you do not need to decode JPEGs to pixels to convert them to JPEG XLs. Rather than relying on the JPEG internal representation (DCT coefficients), utilize JPEG XL directly. Even though only the subset of JPEG XLcorresponds to JPEG is used, the converted images would be 20 percent smaller.

The JPEG XL files, on the other hand, are exact copies of the original JPEG files. Additionally, the conversion is reversible: you can convert JPEG XL files back to JPEGs with minimal computation - so fastyou can do it right on the fly.
...
In terms of image compression, JPEG XL has a high perceptual qualityranges from visually near-lossless (in a side-by-side comparison) to visually completely lossless (in a flicker test, which is stricter than a side-by-side evaluation) to mathematically lossless.

There has been a great deal of effort to preserve the images' subtle texture and other fine details. While it is possible to use the codec at lower bitrates, where the degradation becomes more evident, it excels at the relatively high bitrates.

A video codec-based image format, on the other hand, tends to do well atlow bitrates: it can produce an excellent image ina few bytes. The picture looks good at first but, on closer inspection, often seems washed out. Usually,won't be a problem for video codecs: low bitrates keep the file size or bandwidth low, and frames are typically shown for less than 40 milliseconds, so the audience usually won't notice such artifacts. Still images, however, are often of higher quality.
...
The JPEG XL Project took into account those transition problems and designed the format as best as possible to overcome them. The JPEG XL format is a superset of the JPEG format. In addition, JPEG XL developers did not change any of the JPEG coding tools, even with its numerous under-the-hood coding toolsachieve superior compression. Therefore, all JPEG image data is represented as JPEG XL.
...
There can be a "reference frame"can be used as a basis for working with other frames. This makes it possible to encode repeating elements only once and reuse them repeatedly.
...
When predicting the value of a pixel in position (x,y), the adaptive predictor identifies the prediction modeis most likely to produce the least amount of error by calculating the maximum error expected for pixels (x-1, y), (x,y, -1) and (-1, y -1). An adaptive predictorcomputes the expected error in this way also keeps track of the number of prediction modes expected to reach the level of accuracy calculated.

Multiple distributions are created based on the expected error and the number of "correct" prediction modes. This technique detects flat areas in an image, uses specific distributions, and achieves significant size savings.
...
As the known coefficients are extended to have size, the missing coefficients are filled with zeros, starting with a known coefficient. Then it applies the inverse procedure described in Lemma 4.1 to each block sothe DC image is 2\* upsampled.
...
Although JPEG XL delivers substantial improvements in artifact reduction, block boundaries and ringing can still be seen, particularly at lower qualities. Ringing artifacts are spurious signalsoccur in sharp edges when high-frequency components are quantized or truncated. JPEG XL uses two loop filters to reduce their impact following the decompression process.
...
Moreover, the JPEG XL format allows the decoder to add a quantization constraint to the output of this filter to improve the detail preservation of the image: The DCT step is repeated, and the decoder ensuresthe resulting coefficients do not exceed the value from the bitstream which was quantized by clamping. After applying an IDCT step, the decoder produces the final output of the filter.
...
In the encoding of AC coefficients, the order of encoding is followed; it keeps track of the number of non-zero coefficients in the current block, and uses this value, along with the position in the current block, to split the coefficients into multiple distributions. Whenever there are no non-zero coefficients left in the bitstream, no further values are present.
...
10. The printing industry widely uses CMYK color models. In addition, spot colors can be applied in additional channels. To distribute images intended for printing, high-resolution and lossless orhigh quality are required, as is the capability to show CMYK and possibly additional channels (for spot colors/OGV). As CMYK is generally used for interchange rather than storage or delivery, its storage will typically be lossless or near-lossless.
...
- In the DCT, coefficients are reordered somore blocks have longer series of zeros at the end. In contrast to limited RLE, the index of the last non-zero coefficient is encoded explicitly.
