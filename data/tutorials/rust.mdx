---
blog: true
title: Rust JPEG XL support
description: Rust is a multi-paradigm, high-level, general-purpose programming language. Learn how to use JXL images within Rust.
keyword: Rust
category: tutorials
subcategory: browser
support: full support
url: tutorials/rust/
image: "/logo_draft.png"
datePublished: 31.10.21
dateModified: 31.10.21
sources:
  - github.com/inflation/jpegxl-rs
  - docs.rs/jpegxl-rs/0.6.0/jpegxl_rs/
  - www.rust-lang.org/
  - en.wikipedia.org/wiki/Rust_(programming_language)
tags:
  - jpeg xl
  - jxl
  - image format
  - rust
  - jpeg xl rust
  - crate
questions:
  - Does Rust support JXL?
relatedPosts:
  - rust
  - squoosh
  - windows
---

## What is Rust?

As a fast and memory-efficient language, Rust does not require a runtime or garbage collector, so it can be used to run embedded devices, power performance-critical web services, and easily integrate with other languages. Rust's rich type system and ownership model guarantees memory safety and thread safety, enabling you to eliminate many categories of bugs before they are compiled. There is excellent documentation, an excellent compiler with helpful error messages, and top-quality tooling - an integrated package manager and build tool, superb support for multi-editors with auto-completion and type inspection, as well as an auto-formatter.

## Rust JXL support

The crate JPEGXL_RS offers a safe JPEGXL wrapper over jpeg-xl library, based on the official libjxl.

https://docs.rs/jpegxl-rs/0.6.0/jpegxl_rs/
