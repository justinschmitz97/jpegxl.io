...
const FilesList = (props: FilesListProps) => {
    ...
    const downloadZip = () => {
        ...
        for (let i = 0; i < props.files.length; i++) {
            zip.file(props.files[i].name + '.jxl', new Blob([props.files[i].converted], {type: "octet/stream"}), {binary: true});
        }
    ...
    return (
        <div>
            <div className={styles.listContainer}>
                <Button onClick={downloadZip} style={{visibility: zipButtonVisibility}} className={styles.downloadZipButton} variant="contained" startIcon={<DownloadIcon />}>
                    Download zip
                </Button>
