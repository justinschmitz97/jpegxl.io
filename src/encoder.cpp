...
std::unique_ptr<IEncoder> create_encoder(const uintptr_t data, const size_t size, jxl::CodecInOut* io, jxl::ThreadPoolInternal* pool)
{
    ...
    {
        uint32_t avif_size, width, height;
    ...
    if (WebPGetInfo(data_buffer, size, nullptr, nullptr))
    {
        uint32_t webp_size, width, height;
        auto webp_image = decode_webp(data_buffer, size, webp_size, width, height);

        if (webp_image)
            return std::make_unique<JXLExternalEncoder>(std::move(webp_image), webp_size, width, height);
    }
    return nullptr;
}
...
{
    io->metadata.m.bit_depth.bits_per_sample = depth;
    io->metadata.m.SetAlphaBits(depth);
    if (io->metadata.size.Set(m_width, m_height)) 
    {
        auto* main = &io->Main();
        if (jxl::ConvertFromExternal(
            jxl::Span<const uint8_t>(reinterpret_cast<const uint8_t*>(m_data.get()), m_size), m_width, m_height, 
            jxl::ColorEncoding::SRGB(false), true,
            false, depth, JXL_LITTLE_ENDIAN,
            false, pool, main))
        {
            jxl::PassesEncoderState passes_encoder_state;
            return jxl::EncodeFile(params, io, &passes_encoder_state, compressed, nullptr, pool);
